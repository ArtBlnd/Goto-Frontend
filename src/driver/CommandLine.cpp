...
void SetOptionValue(std::string& Key, std::vector<std::string>& Val, DriverOption* Option)
{
    if (Val.empty())
    {
    ...
    if (Val.size() == 1)
    {
        ...
        if (Language::ttIsInteger(strVal))
        {
            Option->SetValue(Language::tfStr2IntegerTransform(strVal));
            return;
        }

        if (Language::ttIsBoolean(strVal))
        {
...
void ParseArgsImpl(const std::string& argsLine, CommandLineContext* context)
{
    std::string              Key = "";
    std::string              Val = "";
    ...
    for (size_t i = 0; i < argsLine.length(); ++i)
    {
        ...
        if (Language::ttIsDoubleQuote(c) || Language::ttIsSingleQuote(c))
        {
            if (isOnValScope)
            {
        ...
        if (Language::ttIsComma(c) || Language::ttIsSemicolon(c))
        {
            if (isOnValScope)
            {
                if (!Val.empty())
...
bool TryParseCommandLine(CommandLineContext* context, size_t args, const char** argv, const char** envp)
{
    if (args == 1)
    {
